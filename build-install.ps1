...
# 编译输出路径（Test 模式用 Release 路径检查）
$BuildConfig = if ($Configuration -eq "Test") { "Release" } else { $Configuration }
$OutputPath = "Output\$BuildConfig\CursorWorkspaces\net8.0-windows"
...
if ($Configuration -eq "Test") {
    Write-Host "（Test 模式：仅编译验证，不安装）" -ForegroundColor Gray
}
Write-Host ""
...
# 2. 编译项目
Write-Host "[2/8] 正在编译项目 ($BuildConfig)..." -ForegroundColor Yellow
dotnet build -c $BuildConfig --no-restore
if ($LASTEXITCODE -ne 0) {
...
if ($Configuration -eq "Test") {
    Write-Host "========================================" -ForegroundColor Cyan
    Write-Host "Test 模式完成：编译与输出检查通过，未执行安装。" -ForegroundColor Green
    Write-Host "========================================" -ForegroundColor Cyan
    exit 0
}
